...
class Dataset:
    ...
    def dv_format(self) -> dict:
        ...
        fields = result["datasetVersion"]["metadataBlocks"]["citation"][
            "fields"
        ]
        fields.append(primitive(self.title, "title"))
        fields.append(compound(self.authors, "author",
                               ["authorName", "authorAffiliation",
                                "authorIdentifierScheme", "authorIdentifierValue"]))
        fields.append(compound(self.contacts, "datasetContact",
                               ["datasetContactName", "datasetContactEmail",
                                "datasetContactAffiliation"]))
        fields.append(compound(self.description, "dsDescription",
                               ["dsDescriptionValue"]))
...
def create_from_dict(data: dict) -> Dataset:
    ...
    contact = Contact(datasetContactName=data["contactName"],
                      datasetContactEmail=data["contactEmail"])
    description = Description(dsDescriptionValue=data["description"])
    return Dataset(title=title, authors=[author], contacts=[contact],
                   description=[description], subjects=data["subjects"])
...
def compound(values: List, type_name: str, subtype_names: List[str]) -> dict:
    ...
    for v in values:
        result["value"].append({
            ...
            for subtype in subtype_names if getattr(v, subtype) is not None
        })
